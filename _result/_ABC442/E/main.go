...
func main() {
	...
	monster := make([]float64, N)
	for i := 0; i < N; i++ {
		x := float64(nextInt())
		y := float64(nextInt())
		monster[i] = math.Atan2(x, y)
	}

	sorted_monster := make([]float64, N)
	copy(sorted_monster, monster)
	sort.Slice(sorted_monster, func(i, j int) bool {
		return sorted_monster[i] > sorted_monster[j]
	})
	...
	for i := 0; i < Q; i++ {
		...
		res := 0
		for j := 0; j < N; j++ {
			if sorted_monster[j] >= before && sorted_monster[j] <= after {
				res++
			}
		}
		out(res)
